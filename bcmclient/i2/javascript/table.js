  ...
  var table_sorters = new Array();
  function table_createSorter(tableId, sortBy, sortOrder)
  {
    var i =table_sorters.length;
    table_sorters[i] = new Array();
    table_sorters[i][0] = tableId;
    table_sorters[i][1] = new TableSorter(tableId);
    if (sortBy != null && sortBy!='') table_doSort( sortOrder,sortBy, tableId, false);
  }

  function table_getSorter(tableId)
  {
    for(i=0;i<table_sorters.length;i++)
    {
      if (table_sorters[i][0] == tableId) return table_sorters[i][1];
    }   return null;
  }
...
//    alert("date");
  ...
  function exportTablePGL(tableId)
  {
    ...
    {
       core_alert("Your search Criteria resulted in {0} rows. Only {1} rows can be exported to excel. Please change the search critieria and try again.", recordCountElem.value, max);
       return;
    }
    
    //RIC:243997 Praneeth Added popups Start
    
    var res1 = core_confirm("Exporting record(s) to Excel can take an extended amount of time for a single request. Do you wish to continue?");
    if(res1 == "yes")
    {
    var res2 = core_confirm("Do NOT select Export to Excel again until you get the popup window to save. Multiple requests will lead to ISCP system failure. Do you wish to begin exporting?");
     if(res2 == "yes")
      {
	    if(maxRowsElem != null)
	      maxRowsElem.value = recordCountElem.value;

	    document.form.BUTTON_ID.value = "SYS_RELOAD";
	    document.form.target = "i2ui_shell_bottom";    
	    document.form.action=omxContextPath+ "/SearchDownloadServlet?FILTER_ID="+ tableId+"&PAGE_ID="+pageId;
	    document.form.submit();
    ...
    //RIC:243997 Praneeth Added popups End
  }
...
function apply_filters(tableId)
{
  ...
  if (bValidFilters)
  {
    ...
    ui_submit_param("SYS_RELOAD","innerAction=Filter");
  }
}
