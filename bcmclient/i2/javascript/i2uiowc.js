...
function i2uiStylizePivot(pivot, view, id, h, w, hasTotals)
{
  ...
  setTimeout("i2uiBestFitPivot('"+id+"',"+h+","+w+")",250);
}
...
function i2uiInvokeAppMenu(pivot, measureName, menuindex, nameValues)
{
  ...
  if (itemnode != null)
  {
    ...
    var xpath = "/pivot/menu[@owner='"+itemnode.getAttribute("name")+"']";
    //alert(xpath);
...
function i2uiBuildPivotXML(xml, pivot, id, viewname)
{
  ...
  for (var i=0; i<itemnodes.length; i++)
  {
    ...
    attr.text = itemnodes[i].getAttribute("name");
  ...
  if (flatdatanode != null)
  {
    ...
    for (var i=0; i<rowlen; i++)
      dataNode.appendChild(rownodes[i].cloneNode(false));
  }
  else
  {
