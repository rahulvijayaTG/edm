    ...
    if ( checkifAnySelected(result_form) == true )
        ...
        if(checkType(result_form))
        {
            ...
            if(totalRecordCount > exportToExcelThreshold)
                ...
                {
                    exportAll = 'NO';
                    document.result_form.action=omxContextPath+ "/bcm/framework/util/exportToExcel.cmd?SELECTED_ROW_NUMS="+selRows+"&EXPORT_ALL="+exportAll+"&FILE_NAME="+fileName+"&FILE_FORMAT="+fileFormat;
                    document.result_form.submit();
                }
            }
            else
...
function groupEditAll()
{
    ...
    document.result_form.action= omxContextPath+ "/bcm/framework/forms/formController/goToGroupEditPage.cmd";
    document.result_form.submit();
}
