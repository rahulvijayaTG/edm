...
{
    var rc = i2uiShowMessageBox(omxContextPath + "/core/prompt/controller/display.x2c?FIELD_LABEL="+escape(fieldLabel)+"&PAGE_TITLE="+escape(pageTitle)+"&TYPE=success",150,350);
    return rc ;
...
{
    var rc = i2uiShowMessageBox(omxContextPath + "/core/alert/controller/display.x2c?MESSAGE="+escape(msg)+"&TYPE=success",150,350);
    return rc ;
...
function core_alert(msg)
{
  ...
  { 
    arg_val = i-1;
    msg = msg+"&ARG"+arg_val+"="+arguments[i];    
  }
  //RIC:208807: PKarnam: removing escaping the message to make sure argument values are embedded properly
  var rc = i2uiShowMessageBox(omxContextPath + "/core/alert/controller/display.x2c?MESSAGE="+(msg),150,350);
  return rc ;
}
...
function core_alert_base(msg, i18n, scroll)
{
  ...
  var rc = i2uiShowMessageBox(omxContextPath + "/core/alert/controller/display.x2c?MESSAGE="+escape(msg)+ "&SCROLLING="+scroll + "&I18N="+i18n,150,350);
  return rc ;
}
...
function core_warning(msg)
{
  ...
  var rc = i2uiShowMessageBox(omxContextPath + "/core/alert/controller/display.x2c?MESSAGE="+escape(msg)+"&TYPE=warning",150,350);
  return rc ;
}
...
{
    var rc = i2uiShowMessageBox(omxContextPath + "/core/alert/controller/display.x2c?VALIDATE_LOGIN=NO&MESSAGE="+escape(msg),150,350);
    return rc ;
}
...
function core_confirm(msg)
{
  ...
  var rc = i2uiShowMessageBox(omxContextPath + "/core/alert/controller/display.x2c?MESSAGE="+escape(msg)+"&ALERT_TYPE=CONFIRM",150,350);
  return rc ;
}
...
{
    popUpWindow(omxContextPath + "/core/alert/redirectToConfirm.x2c?MESSAGE="+escape(msg),'popUp5');
}

function omx_confirm(msg,funct)
{
    popUpWindow(omxContextPath + "/core/alert/redirectToConfirm.x2c?MESSAGE="+escape(msg)+"&ACTION="+funct+"&TYPE=OMX_CONFIRM",'popUp6');
}
...
function popUpWindow(url,popUp) {
    ...
    if (popUp=="popUp2") 
        { // popUp2 is used for the address book    
          
            if ( (window.popUp2 != null) && (!window.popUp2.closed) )  // MAB: changed top.window.popUp2 to window.popUp2.
                { 
            ...
            window.popUp2 = window.open(omxContextPath + "/core/blank.jsp",popUp,'height=450, width=600, scrollbars=yes, resizable=yes');

            for (var i=0; i<5000; i++) 
                {
                    if (window.popUp2) 
                        {
                            window.popUp2.location = escapeUrlParamValues(url);
                            i = 5000;
                            }
                    }
        } 
        ...
        { // popUp2 is used for the address book    
            ...
            if ( (window.orderTree != null) && (!window.orderTree.closed) )  // MAB: changed top.window.orderTree to window.orderTree.
                { 
                    var windowURL = window.orderTree.location.href.toString();
                    if (windowURL.indexOf(url)>0) 
                        {
                            window.orderTree.focus();
                            return;
                        } 
                    else 
                        {
                            window.orderTree.close();
                        }
                } 
            
            window.orderTree = window.open(omxContextPath + "/core/blank.jsp",popUp,'height=450, width=600, scrollbars=no, resizable=yes');

            for (var i=0; i<5000; i++) 
                {
                    if (window.orderTree) 
                        {
                            window.orderTree.location = escapeUrlParamValues(url);
                            i = 5000;
                            }
                    }
        } 
        ...
        { 
            ...
            if ( (window.catalogPopup != null) && (!window.catalogPopup.closed) )
                { 
                    var windowURL = window.catalogPopup.location.href.toString();
                    if (windowURL.indexOf(url)>0) 
                        {
                            window.catalogPopup.focus();
                            return;
                        } 
                    else 
                        {
                            window.catalogPopup.close();
                        }
                } 

            window.catalogPopup = window.open(omxContextPath + "/core/blank.jsp",popUp,'height= '+ popH + ', width= ' + popW + ', top=' + topPos + ', left=' + leftPos + ', scrollbars= ' + bScrollbars + ', resizable= ' + bResizable);
            
            for (var i=0; i<5000; i++) 
                {
                    if (window.catalogPopup) 
                        {
                            window.catalogPopup.location = escapeUrlParamValues(url);
                            i = 5000;
                            }
                }
    }
        ...
        { 
            ...
            if ( (window.absePopup != null) && (!window.absePopup.closed) )
                { 
                    if (windowURL.indexOf(url)>0) 
                        {
                            window.absePopup.focus();
                            return;
                        } 
                    else 
                        {
                            window.absePopup.close();
                        }
                }                

            window.absePopup = window.open(omxContextPath + "/core/blank.jsp",popUp,'height= '+ popH + ', width= ' + popW + ', top=' + topPos + ', left=' + leftPos + ', scrollbars= ' + bScrollbars + ', resizable= ' + bResizable);
            
            for (var i=0; i<5000; i++) 
                {
                    if (window.absePopup) 
                        {
                            window.absePopup.location = escapeUrlParamValues(url);
                            i = 5000;
                        }
                }
    }        
        ...
        { 
            ...
            if ( (window.orderPopup != null) && (!window.orderPopup.closed) )
                { 
                    var windowURL = window.orderPopup.location.href.toString();
                    if (windowURL.indexOf(url)>0) 
                        {
                            window.orderPopup.focus();
                            return;
                        } 
                    else 
                        {
                            window.orderPopup.close();
                        }
                } 

            window.orderPopup = window.open(omxContextPath + "/core/blank.jsp",popUp,'height= '+ popH + ', width= ' + popW + ', top=' + topPos + ', left=' + leftPos + ', scrollbars= ' + bScrollbars + ', resizable= ' + bResizable);
            
            for (var i=0; i<5000; i++) 
                {
                    if (window.orderPopup) 
                        {
                            window.orderPopup.location = escapeUrlParamValues(url);
                            i = 5000;
                    }
                }
    }
    else if (popUp=="popUp3") 
        { // popUp3 is used for the save menu, email, and fax
            if (window.popUp3) 
                { 
                    window.popUp3.close(); 
                    } 
            window.popUp3 = window.open(escapeUrlParamValues(url),popUp,'height=275,width=400, scrollbars=yes, resizable=no');
    } 
    else if (popUp=="popUp4") 
        { // popUp4 is used for help window
            popW = 800;
            popH = 600;
            bScrollbars = "yes";
            bResizable="yes";
            ...
            window.popUp4 = window.open(escapeUrlParamValues(url),'popUp4','height= '+ popH + ', width= ' + popW + ', top=' + topPos + ', left=' + leftPos + ', scrollbars= ' + bScrollbars + ', resizable= ' + bResizable);
    }
    else if (popUp=="popUp5") 
        { // popUp5 used for alerts
            popW = 390;
            popH = 130;
            ...
            window.popUp5 = window.open(escapeUrlParamValues(url),popUp,'height=130, width=390, top=' + topPos + ', left=' + leftPos + ', scrollbars=no, resizable=no');
    }
    else if (popUp=="alertPopup") 
        { // alertPopup used for alerts
            popW = 390;
            popH = 130;
            ...
            window.alertPopup = window.open(escapeUrlParamValues(url),popUp,'height=120, width=332, top=' + topPos + ', left=' + leftPos + ', scrollbars=no, resizable=no');
    }
        ...
        { // popUp5 used for alerts
            ...
            window.popUp6 = window.open(escapeUrlParamValues(url),popUp,'height=130, width=390, top=' + topPos + ', left=' + leftPos + ', scrollbars=no, resizable=no');
    }       
    else if (popUp=="planPopUp") 
        { // popUp2 is used for showing history     
            if (window.planPopUp && !window.planPopUp.closed) 
                { 
                    var windowURL = window.planPopUp.location.href.toString();
                    if (windowURL.indexOf(url)>0) 
                        {
                            window.planPopUp.focus();
                            return;
                            } 
                    else 
                        {
                            window.planPopUp.close();
                            }
                } 
            window.planPopUp = window.open(omxContextPath + "/core/blank.jsp",popUp,'height=410, width=800, scrollbars=yes, resizable=yes');
            for (var i=0; i<5000; i++) 
                {
                    if (window.planPopUp) 
                        {
                            window.planPopUp.location = escapeUrlParamValues(url);
                            i = 5000;
                              }
                    }
          } 
    else if (popUp=="lotDetailsPopUp") 
        { // popUp2 is used for adding/viewing serial and lot numbers
            if (window.lotDetailsPopUp && !window.lotDetailsPopUp.closed) 
                { 
                    var windowURL = window.lotDetailsPopUp.location.href.toString();
                    if (windowURL.indexOf(url)>0) 
                        {
                            window.lotDetailsPopUp.focus();
                            return;
                              } 
                    else 
                        {
                            window.lotDetailsPopUp.close();
                            }
                    } 
            window.lotDetailsPopUp = window.open(omxContextPath + "/core/blank.jsp",popUp,'height=350, width=680, scrollbars=yes, resizable=yes');
            for (var i=0; i<5000; i++) 
                {
                    if (window.lotDetailsPopUp) 
                        {
                            window.lotDetailsPopUp.location = escapeUrlParamValues(url);
                            i = 5000;
                            }
                    }
        } 
...
function escapeUrlParamValues(url)
{
  ...
  if (queryStrIndex > -1)
  {
    ...
    for(var i=0; i < keyValuePairs.length; i++) 
    {
      ...
      var value = escape(keyValuePair[1]);
      urlPath += (param + "=" +value);
