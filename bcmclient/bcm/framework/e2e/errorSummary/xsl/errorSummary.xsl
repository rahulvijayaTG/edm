...
<xsl:import href="../../../queryform/xsl/searchformfilter.xsl"/>
  ...
  <xsl:template match="RESPONSES" mode="content">
    ...
    <xsl:call-template name="include_javascript_errorSummary"/>
  </xsl:template>
  ...
  <!-- **********************************************************************
       *********************************************************************** -->
  <xsl:template name="include_javascript_errorSummary">
     <script>
         function resize_Containers()
     {
      ...
      function SetfocusSubmit( target )
      {
       ...
       function startErrorCorrection()
       {
        ...
        var comfirmMsg = 'Do you want to start the error correction workflow?';
        if( core_confirm( comfirmMsg ) == 'yes' )
        {
            document.result_form.action=omxContextPath+ "/bcm/framework/e2e/errorSummary/view/startErrorCorrection.cmd";
            document.result_form.submit();
        }

       }

       function reloadErrorDataToMaster()
       {
        document.result_form.target="appFrame";
        document.result_form.DO_SEARCH.value='Yes';
        document.result_form.START_COUNT.value=0;
        ...
        if( core_confirm( comfirmMsg ) == 'yes' )
        {
            document.result_form.action=omxContextPath+ "/bcm/framework/e2e/errorSummary/view/reloadDataByEntity.cmd";
            document.result_form.submit();
        }
   ...
   <![CDATA[
    ...
    function loadErrorTable(selectedDoc)
    {
        document.result_form.target="appFrame";
        document.result_form.CURRENT_ID.value=selectedDoc;
        document.result_form.DO_SEARCH.value='Yes';
        document.result_form.START_COUNT.value=0;
        document.result_form.action=omxContextPath+ "/bcm/framework/e2e/errorSummary/view/loadErrorTableEditor.cmd";
        document.result_form.submit();
    }
    ]]>
   </script>
