...
var i2uiImageDirectory = "/i2/images";
...
function i2uiResizeScrollableArea(mastertableid, minheight, minwidth, slavetableid, flag, slave2width, column1width, headerheight)
{
      ...
      scrolleritem != null)
  {
         ...
         slavetableid != 'undefined'))
    {
      ...
      if (adjust != 0)
      {
        scrolleritem.style.height = newheight + Math.min(16,adjust);
      }
...
/*
  ...
  Compatibility:  IE, NS6
*/
function i2uiManageTreeTable(tablename, cellname, column, relatedtablenames, name, recurse, relatedroutine)
{
  // NS4 browser does not reflow document so what is the point anyway?
  if (document.layers)
  {
      ...
      if (table.rows[i].cells[column].id.substr(0,cellname.length+10) == "TREECELL_"+cellname+"_")
      {
        ...
        else
        {
          ...
          if (depth2 == depth1 + 2)
          {
            ...
            //i2uitrace(1,newcell);
            i2uiManageTreeTable(tablename,newcell,column,relatedtablenames,name,1);
          }
        }
      }
    }
...
/*
  ...
  Compatibility:  IE, NS6
*/
function i2uiCollapseTreeTable(tablename, depth, relatedtablenames, column)
{
  // NS4 browser does not reflow document so what is the point anyway?
  if (document.layers)
  {
    ...
    for (var i=0; i<len; i++)
    {
      ...
      if (rowdepth == depth)
      {
        ...
        if (img != null)
        {
          ...
          if (childnode == null)
          {
            if (img.src.indexOf("container_loadondemand.gif") == -1)
...
function i2uiTreeTableAction(object_name, action)
{
  ...
  else
  {
    ...
    if (owningtable)
    {
      ...
      //if (owningtable.active)
      if (i2uiActiveTreeNode != null)
      {
        //owningtable.active.className = "tableRow0";
        i2uiActiveTreeNode.className = "tableRow0";
      }
      owningrow.className = "rowHighlight";
      //owningtable.active = owningrow;
      i2uiActiveTreeNode = owningrow;
    }
  }
  eval(action);
...
function i2uiHideTableColumn(id, which)
{
  if (document.layers)
    return;

  var len;
  var header_table_obj;
  var data_table_obj;
  header_table_obj = document.getElementById(id+"_header");
  data_table_obj = document.getElementById(id+"_data");
  if (header_table_obj != null && data_table_obj != null)
  {
    len = header_table_obj.rows.length;  
    for (var i=0; i<len; i++)
      header_table_obj.rows[i].cells[which].style.display = "none";
    len = data_table_obj.rows.length;  
    for (var i=0; i<len; i++)
      data_table_obj.rows[i].cells[which].style.display = "none";
  }
  else
  {
    table_obj = document.getElementById(id);
    if (table_obj != null)
    {
      len = table_obj.rows.length;  
      for (var i=0; i<len; i++)
        table_obj.rows[i].cells[which].style.display = "none";
    }
  }
}
function i2uiShowTableColumn(id, which)
{
  if (document.layers)
    return;
  ...
  if (header_table_obj != null && data_table_obj != null)
  {
    len = header_table_obj.rows.length;  
    for (var i=0; i<len; i++)
      header_table_obj.rows[i].cells[which].style.display = "";
    len = data_table_obj.rows.length;  
    for (var i=0; i<len; i++)
      data_table_obj.rows[i].cells[which].style.display = "";
  }
  else
  {
    table_obj = document.getElementById(id);
    if (table_obj != null)
    {
      len = table_obj.rows.length;  
      for (var i=0; i<len; i++)
        table_obj.rows[i].cells[which].style.display = "";
    }
  }
}
